package com.carrotsearch.javastructs.processor;
 ...
 */
@SupportedAnnotationTypes("com.carrotsearch.javastructs.annotations.Struct")
...
public class StructProcessor extends AbstractProcessor {
  ...
  private void validateType(TypeElement e) throws IOException {
    ...
    List<Predicate<TypeElement>> predicates = Lists.newArrayList(
        validateTypePublic, validateTypeFinal, validateTypeExtendsObject,
  ...
  private Predicate<TypeElement> validateTypeHasArgLessPublicConstructor = new Predicate<TypeElement>() {
    public boolean apply(TypeElement type) {
      for (ExecutableElement constructor : ElementFilter.constructorsIn(type
          .getEnclosedElements())) {
        if (constructor.getModifiers().contains(Modifier.PUBLIC)
            && constructor.getParameters().isEmpty()) return true;
      }
      return false;
    }
  ...
  private void processType(TypeElement type) throws IOException {
    ...
    String packageName = elements.getPackageOf(type).getQualifiedName()
        .toString();
  ...
  public class FieldHelper {
    ...
    public String pluralize(String fieldName) {
      return new org.jvnet.inflector.lang.en.NounPluralizer()
          .pluralize(fieldName);
    }
  }
